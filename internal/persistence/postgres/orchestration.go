...
ORDER BY r.sequence,i.key`)
	...
	for rows.Next() {
		...
		var content string
		if err := rows.Scan(&revision.ID, &revision.Sequence, &revision.Digest, &content, &revision.CreatedAt,
			&revision.CreatedBy, &revision.IdempotencyKey, &revision.Fingerprint); err != nil {
			rows.Close()
			return state, err
		}
...
func (s *OrchestrationState) CreateRevision(
	ctx context.Context,
	createdBy, key, fingerprint string,
	content json.RawMessage,
	now time.Time,
) (orchestrationapi.PersistedRevision, error) {
	...
	if existing, found, err := revisionByDigest(ctx, tx, digest); err != nil {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO cc_idempotency_keys(scope,key,fingerprint,resource_id,created_at)
VALUES ('revision',$1,$2,$3,$4)`, key, fingerprint, existing.ID, now.UTC()); err != nil {
			return orchestrationapi.PersistedRevision{}, err
		}
	...
	revision := orchestrationapi.PersistedRevision{
		ID: "rev-" + randomHex(12), Sequence: sequence, Digest: digest,
		Content: append(json.RawMessage(nil), content...), CreatedAt: now.UTC(), CreatedBy: createdBy,
...
VALUES ($1,$2,$3,$4::jsonb,$5,$6)`, revision.ID, int64(revision.Sequence), revision.Digest, string(revision.Content), revision.CreatedAt, revision.CreatedBy)
	if err != nil {
		return orchestrationapi.PersistedRevision{}, err
	}
	_, err = tx.ExecContext(ctx, `INSERT INTO cc_idempotency_keys(scope,key,fingerprint,resource_id,created_at)
VALUES ('revision',$1,$2,$3,$4)`, key, fingerprint, revision.ID, now.UTC())
	if err != nil {
		return orchestrationapi.PersistedRevision{}, err
	}
...
SELECT r.id,r.sequence,r.digest,r.content::text,r.created_at,r.created_by,i.fingerprint
FROM cc_idempotency_keys i JOIN cc_config_revisions r ON r.id=i.resource_id
WHERE i.scope='revision' AND i.key=$1`, key).Scan(
		&revision.ID, &revision.Sequence, &revision.Digest, &content, &revision.CreatedAt, &revision.CreatedBy, &fingerprint,
	)
...
func revisionByDigest(ctx context.Context, tx *sql.Tx, digest string) (orchestrationapi.PersistedRevision, bool, error) {
	var revision orchestrationapi.PersistedRevision
	var content string
	err := tx.QueryRowContext(ctx, `SELECT id,sequence,digest,content::text,created_at,created_by
FROM cc_config_revisions WHERE digest=$1`, digest).Scan(
		&revision.ID, &revision.Sequence, &revision.Digest, &content, &revision.CreatedAt, &revision.CreatedBy,
	)
	if errors.Is(err, sql.ErrNoRows) {
		return revision, false, nil
	}
...
func (s *OrchestrationState) CreateChange(ctx context.Context, requested orchestrationapi.PersistedChange) (orchestrationapi.PersistedChange, bool, error) {
	...
	_, err = tx.ExecContext(ctx, `
INSERT INTO cc_policy_decisions
  (id,policy_id,effect,risk,reason,minimum_approvals,approval_permission,distinct_actors,prohibit_requester,evaluated_at)
VALUES ($1,$2,$3,$4,$5,$6,NULLIF($7,''),$8,$9,$10)`, decisionID, d.PolicyID, string(d.Effect), string(d.Risk), d.Reason,
		d.Requirement.Minimum, d.Requirement.Permission, d.Requirement.DistinctActors, d.Requirement.ProhibitRequester, requested.Snapshot.UpdatedAt)
	if err != nil {
		return orchestrationapi.PersistedChange{}, false, err
	}
	_, err = tx.ExecContext(ctx, `
INSERT INTO cc_changes
...
VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,NULLIF($10,''),$11,$12,$13,$13)`,
		requested.Snapshot.ID, requested.Snapshot.Action, requested.Snapshot.Requester, requested.Snapshot.RevisionID,
		decisionID, string(requested.Snapshot.Risk), string(requested.Input), requested.IdempotencyKey, requested.Fingerprint,
		requested.JobID, string(requested.Snapshot.State), int64(requested.Snapshot.Version), requested.Snapshot.UpdatedAt)
	if err != nil {
		return orchestrationapi.PersistedChange{}, false, err
	}
	_, err = tx.ExecContext(ctx, `INSERT INTO cc_idempotency_keys(scope,key,fingerprint,resource_id,created_at)
VALUES ('change',$1,$2,$3,$4)`, requested.IdempotencyKey, requested.Fingerprint, requested.Snapshot.ID, requested.Snapshot.UpdatedAt)
	if err != nil {
		return orchestrationapi.PersistedChange{}, false, err
	}
	if err := tx.Commit(); err != nil {
...
UPDATE cc_changes SET state=$2,version=$3,updated_at=$4,job_id=NULLIF($5,'')
WHERE id=$1 AND version <= $3`, updated.Snapshot.ID, string(updated.Snapshot.State), int64(updated.Snapshot.Version),
		updated.Snapshot.UpdatedAt.UTC(), updated.JobID)
	if err != nil {
		return err
	}
	...
	for _, approval := range updated.Snapshot.Approvals {
		...
		_, err = tx.ExecContext(ctx, `
INSERT INTO cc_change_approvals(change_id,actor,permissions,approved_at)
VALUES ($1,$2,ARRAY(SELECT jsonb_array_elements_text($3::jsonb)),$4)
ON CONFLICT (change_id,actor) DO NOTHING`,
			updated.Snapshot.ID, approval.Actor, string(permissions), approval.ApprovedAt.UTC())
		if err != nil {
			return err
		}
	}
...
func scanPersistedChange(row scanner) (orchestrationapi.PersistedChange, error) {
	...
	var input, risk, state, effect, decisionRisk string
	var permissions string
	err := row.Scan(
		&persisted.Snapshot.ID, &persisted.Snapshot.Action, &persisted.Snapshot.Requester,
		&persisted.Snapshot.RevisionID, &risk, &state, &persisted.Snapshot.Version, &persisted.Snapshot.UpdatedAt,
		&input, &persisted.IdempotencyKey, &persisted.Fingerprint, &persisted.JobID,
		&effect, &decisionRisk, &persisted.Snapshot.Decision.Reason,
		&persisted.Snapshot.Decision.Requirement.Minimum, &permissions,
		&persisted.Snapshot.Decision.Requirement.DistinctActors, &persisted.Snapshot.Decision.Requirement.ProhibitRequester,
		&persisted.Snapshot.Decision.PolicyID,
	)
	if err != nil {
		return persisted, err
	}
